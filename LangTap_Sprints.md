...
**Goal:** Authentication and transactional emails land in inboxes, not spam. Email templates match LangTap's visual identity. Notification opt-in prompt built so the drip sequence has eligible users. Onboarding drip sequence encourages activation. Consent state stays consistent in both directions between Supabase and Brevo.
**Status:** Pending
...
**Transactional vs marketing (read before starting):** these are two separate email streams and must never be mixed. Transactional email (password reset, email change, sign-up confirmation if enabled) is sent by Supabase Auth relayed through Brevo SMTP, goes to every user regardless of opt-in, and its templates live in the Supabase dashboard (Authentication > Email Templates), not in Brevo. Marketing email (the drip sequence) is sent by Brevo automations to opted-in contacts only, and its templates live in Brevo. Opting out of marketing must never block a password reset.

**Consent dependency:** The `notifications_enabled` column on `profiles` defaults to `false` (see `docs/BACKEND.md`). Without a user-facing opt-in prompt, the drip sequence has zero eligible users. The opt-in prompt (deferred from Sprint 3 per `docs/AUTH.md`) must be built in this sprint before the drip sequence. The prompt task below also adds `notification_prompt_seen_at` so the prompt shows once per account across devices.

**Email confirmation:** currently disabled in Supabase Auth (see `docs/AUTH.md`). This sprint now contains an explicit decision task for it (second task below); the template task covers the confirmation template only if it is enabled.

**Suggested order:** provider setup (owner) can run in parallel with the code tasks; otherwise top to bottom: confirmation decision gates the template work, the opt-in prompt gates the contact sync, the contact sync gates the drip, the sync-back closes the loop last.
...
| Decide email confirmation state | **Small** | **To Do (OWNER decision, then AI if enabling)** | Confirmation is currently OFF: sign-up returns a live session immediately and unverified addresses enter the database. Decide before soft launch. Option A - keep disabled for soft launch (recommended for 5-10 known testers): zero sign-up friction; downside is that typo'd addresses can opt in to marketing email and hard-bounce, which hurts the new domain's sender reputation while it warms up (bounces are visible in Brevo > Statistics, watch them). Option B - enable: Supabase dashboard > Authentication > Sign In / Providers > Email > turn on "Confirm email". If enabled, the AI must then build: a "check your inbox" state on the sign-up card (sign-up no longer returns a session), a resend-confirmation button (Supabase `resend` API, respect its rate limits), calm error copy for log-in attempts with an unconfirmed address, and verification that the confirmation link routes through `/auth/callback` into onboarding correctly. Recommendation: Option A for soft launch, enable before public launch. Record the decision in this cell when made. Docs: `docs/AUTH.md` (confirmation state, rationale, and flow if enabled). |
| Customise Supabase Auth email templates | **Small** | **To Do** | These are the transactional templates: "Reset Password", "Change Email Address", and "Confirm signup" (only if confirmation is enabled above). They are edited in the Supabase dashboard under Authentication > Email Templates as HTML with Go template variables (e.g. `{{ .ConfirmationURL }}`); they are NOT Brevo templates - Brevo only relays them over SMTP. Steps: (1) AI drafts one shared HTML shell: inline CSS only (email clients strip stylesheets), sage green accent on cream per the app palette, system font stack, LangTap wordmark as styled text not an image, one clear button plus a plain-text URL fallback underneath, no external images (nothing gets blocked or flagged). (2) AI drafts copy per template: plain language, no urgency phrasing that trips spam filters. (3) AI checks the HTML into `docs/email-templates/` so the dashboard is never the only copy. (4) OWNER pastes each template into the dashboard and saves. (5) Test: trigger a password reset, check rendering in Gmail web and iPhone Mail, and confirm the mail-tester.com score from the provider task still holds with the new HTML. Docs: `docs/DEVOPS.md` (template locations, how to edit later). |
| Build notification opt-in prompt | **Medium** | **To Do** | Contextual prompt shown after the user's first practice session completes (deferred from Sprint 3 per `docs/AUTH.md`). (1) Migration: add `notification_prompt_seen_at timestamptz` (nullable) to `profiles` so "shown once per account" survives cross-device use and localStorage clears. Verify the existing self-UPDATE RLS policy covers this column and `notifications_enabled`, and that the guard triggers (`guard_username_change`, `guard_membership_change`) do not block them; no new guard needed - both are user-owned preferences. (2) Extend `UserProfile` type + `profile.service.ts` mapping. (3) UI: banner using the PracticeBanner pattern on the home dashboard (not a blocking modal, never interrupts gameplay), shown when the user has completed their first practice session (reuse the first-practice detection pattern from `hooks/useFirstPracticeEvent.ts`) and `notification_prompt_seen_at` is null. Copy: "Want practice reminders and tips? We'll email you occasionally." Yes sets `notifications_enabled = true`; No or dismiss leaves it `false`. Either answer stamps `notification_prompt_seen_at` so it never shows again. (4) Add a "Practice reminders and tips" toggle to profile > account settings wired to the same column, so consent can be withdrawn in-app at any time (GDPR: withdrawing must be as easy as giving). (5) Tests: unit (render conditions, yes/no/dismiss writes, once-only, settings toggle) + integration per CLAUDE.md 5D (column round-trip on own row, cross-user UPDATE denied, guard triggers do not block the two columns). Docs: `docs/AUTH.md` (prompt location, consent lifecycle), `docs/BACKEND.md` (column + usage), `docs/FRONTEND.md` (prompt + settings toggle). |
| Sync opted-in users to Brevo contacts | **Medium** | **To Do** | The bridge between Supabase and Brevo: the drip automation triggers on contacts entering a Brevo list, so opt-in state must flow outward. (1) OWNER: in Brevo create a contact list "LangTap opt-in" and note its list ID; create an API key (Brevo > SMTP & API > API Keys); generate a long random webhook secret. Add `BREVO_API_KEY`, `BREVO_LIST_ID`, and `SUPABASE_WEBHOOK_SECRET` to Vercel env vars (server-side only, never NEXT_PUBLIC) and `.env.example` placeholders. (2) AI: route handler `app/api/webhooks/notifications/route.ts` receiving a Supabase Database Webhook (dashboard > Database > Webhooks: table `profiles`, event UPDATE, send the secret in an Authorization header). Handler: constant-time secret compare (401 otherwise), no-op when `notifications_enabled` did not change between old and new record, on false-to-true upsert the contact (email + optional attributes: username, JLPT level) into the list, on true-to-false remove the contact from the list. Idempotent (upsert and remove are safe to repeat). Return 200 fast; a Brevo outage must never break the user's profile update - log the failure and return 200 so Supabase does not retry forever (decision: accept rare missed syncs; the settings toggle re-fires on next change). (3) Tests: unit for the route (bad secret 401, no-change no-op, opt-in upsert payload, opt-out removal, Brevo error path swallowed). Docs: `docs/BACKEND.md` (webhook flow), `docs/SECURITY.md` (Section 10 inventory row: new secret-gated mutating surface), `docs/DEVOPS.md` (env vars, webhook setup steps). |
| Build onboarding drip sequence in Brevo | **Medium** | **To Do (OWNER + AI)** | Three-email drip as a Brevo Automation. Trigger: contact added to the "LangTap opt-in" list - this IS the Day 0 moment, because the sync task adds contacts exactly when `notifications_enabled` flips true (satisfies the "not on sign-up" rule by construction). Sequence: Day 0 welcome ("You're in! Here's how to start practising"), wait 2 days, Day 2 activation nudge ("Have you tried kana or kotoba practice yet? Here's a quick tip"), wait 5 days, Day 7 social proof ("Other learners at your JLPT level are practising X minutes a day" - keep it honest: use a real stat once analytics provide one, or reframe as a tip until then). AI drafts subject lines, copy, and HTML using the shared shell from the template task; OWNER builds the automation in Brevo (Automations > Create > entry point "Contact added to a list", alternate email and delay steps) and sends test emails to themselves before activating. Every email carries Brevo's built-in unsubscribe link (legally required; Brevo inserts and enforces it). Set the automation exit condition so contacts who unsubscribe or leave the list mid-sequence exit immediately. Transactional email is unaffected by any of this. Docs: `docs/DEVOPS.md` (automation structure, how to edit copy later). |
| Unsubscribe sync-back from Brevo | **Medium** | **To Do** | Closes the consent loop. When a user unsubscribes via the email footer link, Brevo blocklists them and stops sending, but `notifications_enabled` in Supabase stays true - the app would show them opted in and the sync task would keep trying to re-add them. (1) OWNER: Brevo > Settings > Webhooks (Marketing): create a webhook for the "unsubscribed" and "hard bounce" events pointing at `https://<your-domain>/api/webhooks/brevo?secret=<long-random>`; add the same value to Vercel env as `BREVO_WEBHOOK_SECRET`. (2) AI: route handler `app/api/webhooks/brevo/route.ts`: constant-time secret check on the query param (Brevo does not sign webhook payloads, so the URL secret is the only auth - document this in SECURITY.md), look up the profile by email via the service-role client, set `notifications_enabled = false`. Leave `notification_prompt_seen_at` alone so the prompt does not reappear. On hard bounce also remove the contact from the opt-in list via the Brevo API. Unknown email returns 200 no-op (never leak via status codes which emails exist). Idempotent. (3) Together with the app-to-Brevo sync task, both directions now stay consistent: app changes flow out, email-footer changes flow back. (4) Tests: unit (bad secret 401, unknown email 200 no-op, unsubscribe flips the column, repeat delivery idempotent). Docs: `docs/SECURITY.md` (inventory row), `docs/BACKEND.md` (full consent-loop flow diagram), `docs/DEVOPS.md` (webhook setup). |
...
| Decide lifetime tier and Stripe product structure | **Small** | **To Do (OWNER decision, first gate)** | Blocks the checkout task: lifetime is a one-time payment (Checkout `mode: 'payment'`) while monthly/annual are subscriptions (`mode: 'subscription'`), so the checkout endpoint's shape depends on this. Decide: (1) The lifetime price, or explicitly exclude lifetime from launch and sell monthly/annual only - the schema already supports lifetime via `admin_set_membership` and the voucher backlog task, so it can stay a manual/gift tier at launch. (2) Launch currency: GBP or USD (Stripe can localise later; pick one). (3) Tax: ask an accountant whether UK VAT registration applies at your expected revenue (digital services rules); Stripe Tax can be switched on later, do not block launch on it, but record the decision. (4) Stripe structure: one product "LangTap Membership" with two recurring prices (monthly, annual) plus one one-time price if lifetime ships; record the resulting price IDs - the AI maps price ID to tier in a single server-side constant used by both the checkout route and the webhook handler, never client-side. Record decisions in this cell. Docs: `LangTap_Planning.md` Section 5.15 (fill the Lifetime TBD). |
...
| Set up Stripe account with hosted Checkout Sessions | **Medium** | **To Do** | Create Stripe account. Create membership product(s) per the pricing model. Use Stripe-hosted Checkout Sessions (prebuilt page) instead of custom payment UI. This handles PCI compliance, webhook signature verification, and payment flow out of the box. Wire a checkout session creation endpoint at `app/api/stripe/checkout/route.ts` that calls `stripe.checkout.sessions.create()` server-side and returns the session URL. Client redirects to Stripe's hosted page. Success/cancel URLs point back to the app. Environment: add `STRIPE_SECRET_KEY` and `STRIPE_WEBHOOK_SECRET` to Vercel (Production + Preview) and `.env.example` placeholders; price IDs live in the server-side constant map from the decision task above. Nothing Stripe-related is ever NEXT_PUBLIC: with hosted checkout the client only receives the session URL from the route handler, so no publishable key is needed. The checkout route requires an authenticated non-anonymous user, creates or reuses the Stripe customer and stores `stripe_customer_id` via a service-role path (the `guard_membership_change` trigger blocks client writes to that column), and sets `client_reference_id` to the user id as a second mapping alongside the customer id. Test in Stripe test mode end to end before touching live keys. Docs: update `docs/BACKEND.md` (Stripe endpoints), update `docs/SECURITY.md` (Stripe key handling), update `docs/DEVOPS.md` (Stripe environment variables). |
| Build Stripe webhook handler | **Medium** | **To Do** | Route `app/api/stripe/webhook/route.ts`. Listen for `checkout.session.completed`, `customer.subscription.updated`, `customer.subscription.deleted`. Verify signatures with `STRIPE_WEBHOOK_SECRET` on the raw request body before any parsing; reject 400 on failure. Idempotency spec: migration creates `stripe_events` (id text primary key holding the Stripe event id, type text not null, received_at timestamptz default now()), RLS enabled + FORCE with zero client policies (service role only); handler does `INSERT ... ON CONFLICT DO NOTHING` on the event id and, if no row was inserted, the event was already processed - return 200 without reprocessing. Mapping: resolve the profile via `stripe_customer_id` (fallback `client_reference_id`); resolve tier from price ID via the server-side constant from the decision task. Behaviour: `checkout.session.completed` in payment mode (lifetime) sets tier lifetime with null expiry; `customer.subscription.updated` sets the tier and `membership_expires_at` from `current_period_end` plus a one-day grace; `customer.subscription.deleted` sets tier free (Stripe fires it when the subscription actually ends, including cancel-at-period-end, so no early downgrade). All membership writes go through the existing `admin_set_membership` RPC (service role) so the guard-trigger bypass stays in one place. Return 200 quickly and stay under Stripe's timeout. Docs: update `docs/BACKEND.md` (webhook handler, `stripe_events` table, event processing), update `docs/SECURITY.md` (signature verification, inventory row). |
...
| Write Stripe and daily cap enforcement tests | **Medium** | **To Do** | Test mode checkout flow. Webhook signature verification (bad signature 400, replayed event processed once). Subscription status updates. Membership gate on daily cap removal. Server-side cap enforcement: free user without daily cap event is rejected, paid user bypasses cap check, completion_id validated against cap events. Integration tests per CLAUDE.md 5D: `stripe_events` dedup (same event id twice applies once), membership writes land via the `admin_set_membership` path, RLS deny on `stripe_events` for authenticated and anonymous clients, expired membership treated as free by the enforcement RPC. Docs: no doc update (tests only). |
...
| 2.0 | July 2026 | Sprint 18 and 19 expansion pass (Session 122). Sprint 18 rewritten in depth: transactional vs marketing email streams separated (Supabase Auth templates edited in the Supabase dashboard, Brevo only relays SMTP; drip templates live in Brevo), new email-confirmation decision task (Option A keep disabled for soft launch recommended), opt-in prompt spec'd with a `notification_prompt_seen_at` profiles column + migration + integration tests + in-app settings toggle for GDPR withdrawal, new Brevo contact sync task (Supabase Database Webhook to route handler, secret-gated, `BREVO_API_KEY`/`BREVO_LIST_ID` env vars), drip sequence rebuilt as a Brevo Automation triggered on list entry, new unsubscribe sync-back task (Brevo webhook to `app/api/webhooks/brevo`, flips `notifications_enabled` false, hard-bounce handling). Sprint 19: new first-gate decision task (lifetime tier one-time vs subscription checkout modes, launch currency, VAT/Stripe Tax stance, price-ID-to-tier server-side constant), checkout task expanded (env vars, no NEXT_PUBLIC Stripe values, customer creation via service-role path, `client_reference_id` mapping), webhook task expanded with a full `stripe_events` idempotency table spec (RLS + FORCE, zero client policies, INSERT ON CONFLICT dedup) and per-event behaviour including cancel-at-period-end, tests task expanded with 5D integration coverage. Board header version corrected (was stale at 1.0). |
