...
| Build guest trial cap (15m) | **Medium** | **Done** | `stores/guest-distance.store.ts` (separate persisted store, keyed by gameType). `GUEST_TRIAL_DISTANCE_CAP = 15` in constants. `GuestBanner` now only appears at cap ("You've hit the limit as a guest"). Practice banners with sign-up link shown post-trial. `onCharacterCorrect` disabled when over cap. 4 store tests, 6 banner tests. Session 80. |
