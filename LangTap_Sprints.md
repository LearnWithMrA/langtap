...
| Write Dojo screen spec - Kotoba | Medium | In Progress | See UX_DESIGN.md Section 9. Spec rewritten to Kana-depth structure (§9.1-9.13) and visual shell built at `/dojo/kotoba` with the `variety` fixture. JLPT tab row with roving tabindex, WaniKani-style unit cards (active / completed / locked variants), single-open unit accordion, multi-open level-group accordion, three-row word tile with reserved kanji row for kana-only words, Modal-backed reset popover, parity `state` prop on both dojo clients for deterministic loading / error / empty screens, bare `/dojo` falls through to `not-found`. Awaiting owner review and design tweaks before flipping to Done. |
