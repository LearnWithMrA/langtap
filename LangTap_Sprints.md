...
| Task | Size | Status | Notes |
|---|---|---|---|
| Fix mobile keyboard closing between prompts | **Small** | **To Do** | `type-input.tsx:66` and `swipe-input.tsx:65`: the focus `useEffect` depends on `[disabled, value]`. When `resetInputState()` clears value between prompts, this triggers re-focus, which on mobile causes the keyboard to close and reopen. Fix: remove `value` from the dependency array so focus only fires on `disabled` change. Mobile keyboard stays open between prompt transitions. Desktop input remains focused. Docs: check `docs/FRONTEND.md` input component section. |
| Fix mobile audio not playing without lo-fi | **Small** | **To Do** | `useWordAudio.ts`: Web AudioContext only unlocks on a user gesture. The lo-fi play button provides that gesture, but if the user never taps lo-fi, the AudioContext is never unlocked and word pronunciation audio silently fails. Fix: add a one-time `pointerdown` listener (same pattern already proven in `useKeySound.ts:89-101`) to unlock the AudioContext on the first user interaction regardless of lo-fi state. Test on iOS Safari. Docs: update `docs/FRONTEND.md` audio section. |
| Darken mnemonic text orange for readability | **Small** | **To Do** | The mnemonic/meaning text in `meaning-reveal.tsx:35` uses `text-feedback-wrong` (`#f59e60`), which is too light against the cream card background (`#faf5e4`). Fix: create a new CSS token `--color-feedback-mnemonic` in `globals.css` with a darker orange (~`#d4782a`). Use `text-feedback-mnemonic` in `meaning-reveal.tsx` only. Do NOT change `--color-feedback-wrong` or the `WRONG_COLOURS` array in `game-window.tsx`. Must pass WCAG 4.5:1 contrast against `#faf5e4`. Docs: update `docs/FRONTEND.md` colour palette section to document the new `--color-feedback-mnemonic` token. |
| Show dual mnemonic in dojo tile detail | **Small** | **To Do** | `tile-detail-popover.tsx` currently shows kana, romaji, and mastery score. Import `DUAL_MNEMONICS` from `data/kana/mnemonics.ts` and look up the character's romaji. If a mnemonic exists, render the `.text` field below the mastery score. Show the full dual text (covers both hiragana and katakana in one story, e.g. "あ is an apple with a stem, and ア is an axe cutting it"). Only for characters that have a mnemonic entry. Docs: update `docs/FRONTEND.md` dojo section, check `docs/CONTENT.md` mnemonics section. |
| Remove stale duplicate route placeholders | **Small** | **To Do** | `app/(main)/practice/page.tsx` returns `null` (stale placeholder from before E1 route split). Real redirect lives at `app/(main)/(scene)/practice/page.tsx`. Also `app/(main)/credits/page.tsx` is a stale placeholder while real page is `app/credits/page.tsx`. Flag both stale files for owner deletion. Run `next build` to verify no route conflicts after removal. Docs: no doc update (cleanup only). |
| Normalise special character IDs across docs | **Small** | **To Do** | `docs/CONTENT.md` uses `hira-sokuon` while `docs/GAME_DESIGN.md` uses `h-sokuon` for the same character. Check the actual IDs in `data/kana/characters.ts` and normalise both docs to match the code. Also check `kata-sokuon` vs `k-sokuon` and long vowel IDs. Docs: update `docs/CONTENT.md` and `docs/GAME_DESIGN.md` to match `data/kana/characters.ts`. |
| Wire home leaderboard glance to real data | **Small** | **To Do** | `game-home-client.tsx` uses `EMPTY_LEADERBOARD` (line ~60) and passes it to mode panels (line ~102). Sprint 2B promised a leaderboard glance on the dashboard. The leaderboard hook (`useLeaderboard`) already exists from Sprint 9. Wire it to load top-3 summaries for the home dashboard, or simplify to show the user's rank only. Handle loading and guest states. Docs: update `docs/FRONTEND.md` (dashboard, leaderboard glance). |

---

## Sprint 12 - Reset Progress Upgrade

**Goal:** Reset progress becomes a full factory reset. App state returns to identical-to-new-account. Typed confirmation prevents accidental resets. All server-side state cleared atomically.
**Status:** Pending

| Task | Size | Status | Notes |
|---|---|---|---|
| Define factory reset scope and contract | **Small** | **To Do** | Document every piece of state and whether it is cleared or preserved. **Cleared server-side:** kana mastery scores, kana learning scores, kana manual unlocks, word mastery scores, word manual unlocks, word counters, practice sessions, leaderboard scores/events. **Preserved server-side:** daily cap events (if a user hits their daily cap and resets, they remain capped for the day), profile settings (username, email, JLPT level, distance unit, input mode, leaderboard visibility, notifications_enabled, user_tz), membership status. **Cleared client-side:** mastery store, word mastery store, word counter store, unlock store, dialogue/tutorial seen flags (`langtap-dialogues-seen` localStorage), dojo help card tips, streak cache. **Preserved client-side:** settings store (input direction, kotoba input mode, key clicks, etc. are user preferences, not progress). **Epoch ownership:** the factory reset RPC increments both epochs itself in a single transaction. It does NOT call the existing `reset_all_mastery` / `reset_all_word_mastery` RPCs (which also increment epochs). This avoids double-increment. The factory RPC performs the same DELETE/UPDATE operations directly. Verify each item against the Supabase schema and localStorage keys. Output: checklist used by the next two tasks. Docs: read `docs/BACKEND.md` (schema, RPCs), `docs/GAME_DESIGN.md` (mastery, unlock, streak). |
| Build atomic server-side factory reset RPC | **Medium** | **To Do** | New `factory_reset` RPC that clears all server-side state in a single transaction with profile row lock. Performs DELETE operations directly (does NOT call existing `reset_all_mastery` / `reset_all_word_mastery` RPCs to avoid double epoch increment). Deletes: mastery scores, learning scores, manual unlocks, word mastery scores, word manual unlocks, word counters, practice sessions, practice activity events, leaderboard score events, leaderboard scores. Does NOT delete `daily_cap_events` or modify profile settings. Increments both `mastery_reset_epoch` and `word_mastery_reset_epoch` once. Returns both new epoch values. Single RPC avoids half-reset if one call fails. New `services/factory-reset.service.ts` wraps the RPC call. Docs: update `docs/BACKEND.md` (new RPC, reset behaviour, epoch ownership), update `docs/SECURITY.md` (RPC policies). |
| Build client-side factory reset flow with typed confirmation | **Medium** | **To Do** | `reset-progress.tsx`: Add a "Full Reset" option alongside the existing separate Kana/Kotoba resets. Uses typed confirmation (user types "RESET" to confirm, same pattern as the delete account flow in the profile screen). On confirm: calls the `factory_reset` RPC via service, then clears all local state: mastery store (`resetAll`), word mastery store (`resetAll`), word counter store (`resetAll`), unlock store (recompute to re-lock all), dialogue/tutorial tracking (`clearAll()` exported from `useDialogueSeen.ts`), dojo help card tips. Updates both epoch values from RPC response. Non-optimistic: spinner during RPC, local state cleared only on success. Result: app state identical to a fresh account (except daily cap usage for the current day is preserved). Docs: update `docs/FRONTEND.md` (profile screen, reset behaviour). |
| Write factory reset tests | **Small** | **To Do** | Test scope completeness: every state source from the scope checklist is cleared. Test daily cap preservation: `daily_cap_events` rows survive a factory reset. Test atomicity: if RPC fails, no local state is cleared. Test typed confirmation: "RESET" required, case-sensitive. Test epoch values update from RPC response. Test UI: separate Kana/Kotoba resets still work independently alongside full reset. Docs: no doc update (tests only). |
...
| Wire streak to dashboard | **Small** | **To Do** | `game-home-client.tsx:95`: replace `<StreakCalendar heatmap={EMPTY_HEATMAP} streakCount={0} />` with the `useStreak()` hook output. Handle loading state. Show empty calendar for guests (no streak tracking without an account). Docs: update `docs/FRONTEND.md` (dashboard, streak calendar). |
| Write streak tests | **Small** | **To Do** | RPC: server-derived date matches user timezone (read from profiles, not client-passed), characters_practiced increments, duplicate completion_id is idempotent via events table, anonymous users rejected, direct practice_sessions INSERT blocked by RLS. Service: returns date + count pairs. Hook: loading state, empty for guests, streak derivation with count-based intensity. Dashboard: loading state renders, empty calendar for guests. Docs: no doc update (tests only). |
...
| Task | Size | Status | Notes |
|---|---|---|---|
| Disconnect anonymous auth sign-in | **Small** | **To Do** | Remove the `signInAnonymously()` call from `ensureGuestSession()` in `guest-usage.service.ts`. Remove `isAnonymous` handling from `useAuth`. Remove anonymous user middleware rules. Remove all imports and references to these functions across the codebase. Flag the now-inert guest service files for owner deletion. Docs: covered by the documentation cleanup task below. |
| Disconnect guest usage service, store, and hook | **Small** | **To Do** | Remove all imports and references to `services/guest-usage.service.ts`, `stores/guest-usage.store.ts`, and `hooks/useGuestUsage.ts` from `practice-client.tsx`, `guest-banner.tsx`, the `(main)` layout, and anywhere else. Flag the now-inert files for owner deletion. Docs: covered by the documentation cleanup task below. |
| Disconnect guest distance store | **Small** | **To Do** | Remove all imports and references to `stores/guest-distance.store.ts` throughout the codebase. Flag the now-inert file for owner deletion. Docs: covered by the documentation cleanup task below. |
| Disconnect guest-to-account import flow | **Medium** | **To Do** | Remove all imports and references to `services/guest-import.service.ts` and `services/import-snapshot.ts`. Remove import confirmation modal from `AuthInitializer`. Remove pending import banner. Remove `skip_guest_import` / `skip_legacy_import` profile column usage from the UI. Flag the now-inert service files for owner deletion. Docs: covered by the documentation cleanup task below. |
| Repurpose or disconnect GuestBanner | **Small** | **To Do** | Either repurpose `components/layout/guest-banner.tsx` to show a simple "Sign up to save your progress" message for demo users (no cap tracking, no distance), or remove all references and flag the file for owner deletion. Docs: covered by the documentation cleanup task below. |
...
| Flag database cleanup for owner | **Small** | **To Do** | Flag for owner review: `guest_usage` table, `get_or_create_guest_usage` RPC, `increment_guest_usage` RPC, and anonymous-specific RLS policies can be removed via a migration. The AI does not delete per project rules. The owner performs all destructive database operations manually. Docs: update `docs/BACKEND.md` (flag deprecated tables/RPCs). |
| Update all guest-related documentation and fix doc drift | **Large** | **To Do** | Remove or update guest mode references in: `docs/AUTH.md` (guest mode, anonymous auth, guest import), `docs/BACKEND.md` (guest usage table, RPCs, anonymous RLS), `docs/SECURITY.md` (anonymous user policies, guest cap enforcement), `docs/GAME_DESIGN.md` (guest distance cap, trial system), `docs/FRONTEND.md` (guest banner, guest state), `docs/ARCHITECTURE.md` (guest stores, guest services). Replace with demo mode documentation where applicable. **Also fix doc drift discovered in project review:** (1) `docs/BACKEND.md` is missing tables/RPCs added since Sprint 9: `daily_cap_events`, `leaderboard_sessions`, `leaderboard_word_catalog`, `practice_activity_events` (Sprint 13). Audit all migrations in `supabase/migrations/` and ensure every table and RPC is documented. (2) `LangTap_Planning.md` still treats Kotoba as Phase 2 (Sprints 5B-7 already built it). Update to reflect Kotoba is live in Phase 1. (3) `theme/colors.ts` is a placeholder file while actual colour tokens live in `globals.css`. Update `CLAUDE.md` and `docs/FRONTEND.md` to reflect the real source of truth for colours (`globals.css` CSS variables), or populate `theme/colors.ts` from the CSS variables. This is an explicit task, not incidental cleanup. |

### Phase E: Tests

| Task | Size | Status | Notes |
|---|---|---|---|
| Write demo mode tests | **Medium** | **To Do** | Demo kana practice: fixed prompt sequence, no mastery/counter store writes, completion screen appears at end, reset on replay. Demo kotoba practice: same guarantees for readings and kanji mode prompts. Demo kana dojo: fixture data renders with varied states. Demo kotoba dojo: fixture data renders with varied states. "Try it out" resets demo. No anonymous auth calls during demo. Docs: no doc update (tests only). |
| Flag obsolete guest tests for removal | **Small** | **To Do** | Existing guest cap tests, guest import tests, and anonymous auth tests become obsolete after the demo system replaces the guest system. Flag for owner review. Docs: no doc update (flagging only). |
...
| Add terms and conditions checkbox to sign-up | **Small** | **To Do** | `sign-up-card.tsx`: Add a required checkbox below the password field: "I agree to the [Terms of Service](/terms) and [Privacy Policy](/privacy)". Links open in new tabs. Sign-up button disabled until checkbox is checked. Checkbox state is client-side only (not stored in Supabase). The terms and privacy pages already exist (built in Sessions 100-101). Applies to email sign-up only (OAuth providers handle consent via their own flows). Docs: update `docs/AUTH.md` (sign-up flow, terms consent), update `docs/FRONTEND.md` (sign-up card). |
...
| Privacy and legal page sweep | **Medium** | **To Do** | The privacy page (`app/privacy/page.tsx`) still describes guest localStorage and a 30-minute cap (line ~67), and says "no marketing emails" (line ~95). By this sprint, Sprints 14 (email), 15 (demo replaces guest), 16 (bug report screenshots), and 17 (analytics) have added new data handling. Review and update both `app/privacy/page.tsx` and `app/terms/page.tsx` to accurately reflect: demo mode (no anonymous auth, no stored data), email collection and drip sequence (with opt-in), bug report screenshots (stored in Supabase Storage), Vercel Analytics (page views, custom events), and upcoming payments (Sprint 19). Update "Last updated" dates. Docs: no doc update (legal pages are the docs). |
